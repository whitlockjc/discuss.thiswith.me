  ...
  xml.xsl :template, "match" => "/" do
    ...
    xml.html "xmlns" => "http://www.w3.org/1999/xhtml", "lang" => "en" do
      xml.head do
        ...
        xml.title do
          xml.text! "ThoughtSpark.org - Sitemap"
        end
              ...
              xml.div "class" => "span12 content", "id" => "content" do
                xml.div "class" => "page-header single-page-header" do
                  xml.h1 do
                    xml.text! "Sitemap"
                    xml.small do
                      xml.tag! "xsl:value-of", "select" => "count(sitemap:urlset/sitemap:url)"
                      xml.text! "urls"
                    end
                  end
                end
                xml.table "class" => "table table-striped table-bordered" do
                  xml.thead do
                    xml.tr do
                      ...
                      xml.th do
                        xml.text! "Last Modified"
                      end
                      xml.th do
                        xml.text! "Priority"
                      end
                      xml.th do
                        xml.text! "Change Frequency"
                      end
                    end
                  end
                  xml.tag! "xsl:for-each", "select" => "sitemap:urlset/sitemap:url" do
                    xml.tr do
                      xml.td do
                        xml.a do
                          xml.xsl :attribute, "name" => "href" do
                            xml.tag! "xsl:value-of", "select" => "sitemap:loc"
                          end
                          xml.tag! "xsl:value-of", "select" => "sitemap:loc"
                        end
                      end
                      xml.td do
                        xml.tag! "xsl:value-of", "select" => "sitemap:lastmod"
                      end
                      xml.td do
                        xml.tag! "xsl:value-of", "select" => "sitemap:priority"
                      end
                      xml.td do
                        xml.tag! "xsl:value-of", "select" => "sitemap:changefreq"
                      end
                    end
                  end
                end
